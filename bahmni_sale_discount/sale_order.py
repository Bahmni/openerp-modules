...
class sale_order(osv.osv):
  ...
  #  return self.action_view_invoice(cr,uid,ids,context=context)
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            for line in order.order_line:
                if line.product_id.categ_id.parent_id.name == "Drug" and not line.external_order_id:
                    sale_order_item = {'productUuid': line.product_id.uuid, 'comments': line.comments, 'quantity': line.product_uos_qty, 'unit': line.product_uom.name, 'orderUuid': line.order_uuid}
    ...
    'default_quantity':fields.float('Default Quantity Value',digits_compute=dp.get_precision('Account'),readonly=True, states={'draft':[('readonly',False)]}),
    ...
    'discount':fields.function(_amount_all, digits_compute=dp.get_precision('Account'), string= 'Discount ',
        store={
            'sale.order': (lambda self, cr, uid, ids, c={}: ids, ['order_line','discount_percentage', 'chargeable_amount', 'discount', 'calculated_discount', 'default_quantity'], 10),
