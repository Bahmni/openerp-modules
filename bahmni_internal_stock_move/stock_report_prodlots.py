        ...
        select report_without_unit.id,report_without_unit.location_id,prodlot_id,
          ...
          swo.product_min_qty as reorder_level
            ...
            (select max(id) as id, location_id, product_id, prodlot_id, sale_price, life_date, sum(qty) as qty
             from (
                    select -max(sm.id) as id, sm.location_id, sm.product_id, sm.prodlot_id, spl.sale_price, spl.life_date, -sum(sm.product_qty /uo.factor) as qty
                    ...
                    group by sm.location_id, sm.product_id, sm.product_uom, sm.prodlot_id, spl.life_date,spl.sale_price

                    union all

                    select max(sm.id) as id, sm.location_dest_id as location_id, sm.product_id, sm.prodlot_id, spl.sale_price, spl.life_date, sum(sm.product_qty /uo.factor) as qty
                    from stock_move as sm
                    left join stock_production_lot spl on (spl.id = sm.prodlot_id)
                    left join stock_location sl on (sl.id = sm.location_dest_id)
                    left join product_uom uo on (uo.id=sm.product_uom)
                    where sm.state in ('done', 'confirmed')
                    group by sm.location_dest_id, sm.product_id, sm.product_uom, sm.prodlot_id, spl.life_date,spl.sale_price
                  ) as report
